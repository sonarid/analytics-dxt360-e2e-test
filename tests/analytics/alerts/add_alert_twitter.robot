*** Settings ***
Documentation     Test for Adding New Alerts by Twitter Account Threshold
...
...
Select Alert Type
    Input Text          //input[@placeholder="Please enter the alert name"]    Testing Robot Buzz
    Click Element       //label[text()="Based on Twitter Account Threshold"]
...
Select Alert Filtering
    ...
    Click Element                       //input[@placeholder="Please select the post type"]
    Wait Until Page Contains Element    //div[@class="grid grid-cols-3 gap-3"]
    Click Element                       //div/span[text()="Tweet"]
    Press Keys                          None   ESC

    Input Text                          //label[text()="Accounts"]/following-sibling::input        testing_robot
    Input Text                          //label[text()="Total Buzz"]/following-sibling::input        200
...
Check If Alert Data is Correct
    [Arguments]
    SeleniumLibrary.Element Text Should Be    //label[text()="Alert Name"]/following-sibling::p    Testing Robot Buzz
    SeleniumLibrary.Element Text Should Be    //label[text()="Interval"]/following-sibling::p    30 Minutes
    SeleniumLibrary.Element Text Should Be    //label[text()="Alert Type"]/following-sibling::div/div/span    Buzz Threshold
